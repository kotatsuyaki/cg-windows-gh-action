...
class Matrix2 {
  public:
    ...
    Matrix2&
    operator-=(const Matrix2& rhs); // subtract rhs and update this object
...
class Matrix3 {
  public:
    ...
    Matrix3&
    operator-=(const Matrix3& rhs); // subtract rhs and update this object
...
class Matrix4 {
  public:
    ...
    Matrix4&
    operator-=(const Matrix4& rhs); // subtract rhs and update this object
...
inline Matrix3::Matrix3(float xx, float xy, float xz, float yx, float yy,
...
inline void Matrix3::set(float xx, float xy, float xz, float yx, float yy,
                         float yz, float zx, float zy, float zz) {
    m[0] = xx;
    m[1] = xy;
    m[2] = xz;
...
inline Matrix3 Matrix3::operator*(const Matrix3& rhs) const {
    return Matrix3(m[0] * rhs[0] + m[1] * rhs[3] + m[2] * rhs[6],
...
inline Matrix3 operator-(const Matrix3& rhs) {
    return Matrix3(-rhs[0], -rhs[1], -rhs[2], -rhs[3], -rhs[4], -rhs[5],
                   -rhs[6], -rhs[7], -rhs[8]);
}
...
inline Matrix4::Matrix4(float xx, float xy, float xz, float xw, float yx,
...
inline void Matrix4::set(float xx, float xy, float xz, float xw, float yx,
                         float yy, float yz, float yw, float zx, float zy,
                         float zz, float zw, float wx, float wy, float wz,
                         float ww) {
    m[0] = xx;
...
inline Matrix4 Matrix4::operator+(const Matrix4& rhs) const {
    return Matrix4(m[0] + rhs[0], m[1] + rhs[1], m[2] + rhs[2], m[3] + rhs[3],
                   m[4] + rhs[4], m[5] + rhs[5], m[6] + rhs[6], m[7] + rhs[7],
                   m[8] + rhs[8], m[9] + rhs[9], m[10] + rhs[10],
                   m[11] + rhs[11], m[12] + rhs[12], m[13] + rhs[13],
                   m[14] + rhs[14], m[15] + rhs[15]);
}
...
inline Vector4 Matrix4::operator*(const Vector4& rhs) const {
    return Vector4(m[0] * rhs.x + m[1] * rhs.y + m[2] * rhs.z + m[3] * rhs.w,
                   ...
                   m[12] * rhs.x + m[13] * rhs.y + m[14] * rhs.z +
                       m[15] * rhs.w);
}
...
inline Matrix4 operator-(const Matrix4& rhs) {
    return Matrix4(-rhs[0], -rhs[1], -rhs[2], -rhs[3], -rhs[4], -rhs[5],
                   -rhs[6], -rhs[7], -rhs[8], -rhs[9], -rhs[10], -rhs[11],
